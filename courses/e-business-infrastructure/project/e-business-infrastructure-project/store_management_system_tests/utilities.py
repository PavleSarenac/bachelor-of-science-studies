...
def recursive_compare(expected, received, level='root', preprocess_list=None, preprocess_scalar=None):
    ...
    if (isinstance(expected, dict) and isinstance(received, dict)):
        if (sorted(expected.keys()) != sorted(received.keys())):
            ...
            message += "{:<20} +{} -{}\n".format(level, expected_key_set - received_key_set,
                                                 received_key_set - expected_key_set)
            same = False
...
def evaluate_search_test(set_up_data, expected_response, received_response):
    ...
    (message, same) = recursive_compare(expected_response, received_response, preprocess_list=preprocess_list,
                                        preprocess_scalar=preprocess_scalar)
...
def set_up_order_test(with_authentication, authentication_url, customer_url, with_blockchain, customer_keys_path,
                      passphrase):
    def implementation(url, headers, data, files):
        if (with_authentication):
            user_login(True, authentication_url, headers)
...
def evaluate_orders_to_pickup_test(set_up_data, expected_response, received_response):
    ...
    (message, same) = recursive_compare(expected_response, received_response, preprocess_list=None,
                                        preprocess_scalar=preprocess_scalar)
...
def set_up_pickup_order_test(with_authentication, authentication_url, courier_url, order_ids, with_blockchain,
                             courirer_private_key):
    def implementation(url, headers, data, files):
        if (with_authentication):
            user_login(False, authentication_url, headers)
...
def set_up_delivered_id(with_authentication, authentication_url, is_customer, order_ids, with_blockchain,
                        customer_keys_path, passhphrase):
    def implementation(url, headers, data, files):
        if (with_authentication):
            user_login(is_customer, authentication_url, headers)
...
def set_up_customer_headers_with_blockchain(with_authentication, authentication_url, with_blockchain,
                                            customer_keys_path, passphrase):
    def implementation(url, headers, data, files):
        if (with_authentication):
            user_login(True, authentication_url, headers)
...
def set_up_invalid_address(with_authentication, authentication_url, courier_url, with_blockchain, order_ids,
                           provider_url):
    def implementation(url, headers, data, files):
        if (with_authentication):
            user_login(True, authentication_url, headers)
...
def customer_equals(with_blockchain, customer_keys_path, customer_passphrase, provider_url):
    def implementation(set_up_data, expected_response, received_response):
        ...
        if (with_blockchain):
            customer_address = Account.from_key(
                Account.decrypt(load_keys_file(customer_keys_path), customer_passphrase)).address
